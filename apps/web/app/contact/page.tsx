...
export default function ContactPage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await fetch(`${API}/api/v1/billing/enterprise/contact`, {
        ...
        body:JSON.stringify({
          business_name:form.company||"Not provided",
          contact_name:form.name,
          business_email:form.email,
          company_size:"",
          message:`[${form.subject}] ${form.message}`,
          industry:form.subject,
        }),
      });
    } catch {}
    setSent(true);
  ...
  const contacts = [
    {icon:"💼",title:"Sales and Pricing",email:"sales@claustor.com",desc:"Enterprise plans, demos, custom pricing"},
    {icon:"🛠️",title:"Technical Support",email:"support@claustor.com",desc:"Platform issues, API, integrations"},
    {icon:"🔒",title:"Privacy and Data",email:"privacy@claustor.com",desc:"Data requests, GDPR, deletion"},
  ...
  return (
    <div style={{fontFamily:"Inter,system-ui,sans-serif",background:"#FAFBFC",minHeight:"100vh"}}>
      <Nav/>
      <PageHero badge="Get in Touch" title="Contact Us" subtitle="We typically respond within 4 business hours."/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:48}}>
            ...
            {sent ? (
              ...
              <>
                ...
                <button onClick={handleSubmit} disabled={!form.name||!form.email||!form.subject||!form.message}
                  style={{width:"100%",padding:"13px",background:"#0066FF",color:"white",border:"none",borderRadius:10,fontSize:15,fontWeight:700,cursor:"pointer",
                    opacity:(!form.name||!form.email||!form.subject||!form.message)?0.5:1}}>
                  Send Message →
                </button>
                <div style={{fontSize:11,color:"#9CA3AF",textAlign:"center",marginTop:8}}>
                  Goes to hello@claustor.com — response within 4 hours
                </div>
              </>
            )}
          </div>
          <div>
            ...
            {contacts.map(({icon,title,email,desc})=>(
              <div key={title} style={{display:"flex",gap:14,marginBottom:14,padding:"14px 16px",background:"white",border:"1px solid #E5E7EB",borderRadius:12}}>
            ...
            <div style={{background:"#F0F7FF",border:"1px solid #DBEAFE",borderRadius:12,padding:16,marginTop:8}}>
              ...
              <div style={{fontSize:13,color:"#374151",lineHeight:1.7}}>
                DKU Technologies Pvt. Ltd.<br/>Hyderabad, Telangana, India
              </div>
            </div>
          </div>
        </div>
