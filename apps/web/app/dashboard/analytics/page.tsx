      ...
      {data.map(item => (
        <div key={item.label}>
          ...
          <div style={{ height:8, background:C.border, borderRadius:4, overflow:"hidden" }}>
            <div style={{ height:"100%", width:`${maxValue>0?(item.value/maxValue)*100:0}%`, background:item.color, borderRadius:4, transition:"width 0.6s ease" }}/>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ overflowX:"auto" }}>
      <table style={{ borderCollapse:"collapse", width:"100%", fontSize:13 }}>
        ...
        <tbody>
          {clauseTypes.map((ct,i) => (
            <tr key={ct} style={{ borderTop:`1px solid ${C.border}`, background:i%2===0?C.surface:C.bg }}>
              <td style={{ padding:"12px 16px", color:C.body, fontWeight:600, textTransform:"capitalize" }}>
                {ct.replace(/_/g," ")}
              </td>
              {["low","medium","high"].map(rl => {
                const cell = matrix[ct]?.[rl] || { count:0, avg_score:0 };
                const c = getCell(cell.avg_score, cell.count);
                return (
                  <td key={rl} style={{ padding:"10px 20px", textAlign:"center" }}>
                    {cell.count > 0 ? (
                      <div style={{ display:"inline-flex", flexDirection:"column", alignItems:"center", background:c.bg, color:c.text, borderRadius:10, padding:"8px 16px", minWidth:72 }}>
...
function ExpiryTimeline({ timeline }: { timeline:any[] }) {
  if (!timeline?.length) return <div style={{ textAlign:"center", padding:40, color:C.muted, fontSize:14 }}>No contracts expiring in the next 3 years</div>;
  const maxCount = Math.max(...timeline.map(q=>q.count));
  return (
    <div style={{ display:"flex", gap:16, alignItems:"flex-end", overflowX:"auto", padding:"8px 0" }}>
      {timeline.map(q => (
        <div key={q.quarter} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:6, minWidth:72 }}>
          <div style={{ fontSize:13, fontWeight:700, color:C.body }}>{q.count}</div>
          <div title={q.contracts.map((c:any)=>c.title).join(", ")} style={{
            width:52, borderRadius:"6px 6px 0 0",
          ...
          />
          <div style={{ fontSize:11, color:C.muted, textAlign:"center", whiteSpace:"nowrap" }}>{q.quarter}</div>
        </div>
      ))}
    </div>
  );
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const qs = selectedId ? `?contract_id=${selectedId}` : "";
    ...
    Promise.all([
      ...
      setOverview(ov); setHeatmap(hm); setDist(dist); setTimeline(tl); setCp(cp);
    }).catch(console.error)
    .finally(() => setLoading(false));
  }, [selectedId]);

  const selectedContract = contracts.find(c => c.id === selectedId);
  const isFiltered = !!selectedId;
  ...
  return (
    <div style={{ padding:"32px 36px" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28 }}>
        <div>
          ...
          <p style={{ fontSize:14, color:C.muted }}>
            {isFiltered ? `Contract: ${selectedContract?.title}` : "Portfolio intelligence — all contracts"}
          </p>
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          {isFiltered && (
            <Link href={`/dashboard/contracts/${selectedId}`}
              style={{ fontSize:13, color:C.primary, textDecoration:"none", padding:"8px 14px", border:`1px solid ${C.primary}`, borderRadius:8 }}>
              View contract →
            </Link>
          )}
      ...
      {isFiltered && selectedContract && (
        <div style={{ background:C.primaryLight, border:`1.5px solid ${C.primary}30`, borderRadius:12, padding:"14px 20px", marginBottom:24, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", gap:20 }}>
            <div><span style={{ fontSize:12, color:C.primary, fontWeight:600 }}>COUNTERPARTY</span><br/><span style={{ fontSize:14, fontWeight:700, color:C.heading }}>{selectedContract.counterparty||"—"}</span></div>
            <div><span style={{ fontSize:12, color:C.primary, fontWeight:600 }}>VALUE</span><br/><span style={{ fontSize:14, fontWeight:700, color:C.heading }}>{selectedContract.contract_value ? `${selectedContract.contract_currency} ${(selectedContract.contract_value/1000000).toFixed(2)}M` : "—"}</span></div>
            <div><span style={{ fontSize:12, color:C.primary, fontWeight:600 }}>EXPIRY</span><br/><span style={{ fontSize:14, fontWeight:700, color:C.heading }}>{selectedContract.expiry_date ? new Date(selectedContract.expiry_date).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : "—"}</span></div>
            <div><span style={{ fontSize:12, color:C.primary, fontWeight:600 }}>TYPE</span><br/><span style={{ fontSize:14, fontWeight:700, color:C.heading }}>{selectedContract.contract_type||"—"}</span></div>
          </div>
          <button onClick={()=>setSelectedId("")} style={{ background:"none", border:"none", color:C.muted, cursor:"pointer", fontSize:18 }}>×</button>
        </div>
      )}

      {loading ? (
        <div style={{ textAlign:"center", padding:80, color:C.muted }}>Loading analytics...</div>
      ) : (
        <>
          {/* Stat cards */}
          {overview && (
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:16, marginBottom:24 }}>
              <StatCard label="Contracts"     value={overview.contracts.total}       color={C.primary} />
              <StatCard label="Analysed"      value={overview.contracts.analyzed}    color={C.success}  sub="processed" />
              <StatCard label="High risk"     value={overview.risk.high}             color={C.error}    sub={`avg ${overview.risk.avg_score}`} />
              <StatCard label="Total value"   value={`$${overview.value.total_m}M`}  color={C.primary} />
              <StatCard label="Clauses"       value={overview.clauses.total}         color="#6366F1"    sub={`avg risk ${overview.clauses.avg_risk}`} />
              <StatCard label="Auto-renewal"  value={overview.contracts.auto_renewal} color={C.warning} sub="contracts" />
              {!isFiltered && <StatCard label="Expiring 90d" value={overview.contracts.expiring_soon} color={overview.contracts.expiring_soon>0?C.error:C.success} />}
            </div>
          )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:20 }}>
            {overview && (
              <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:24 }}>
                ...
                <BarChart maxValue={overview.contracts.analyzed||1} data={[
                  { label:`High (${overview.risk.high})`,   value:overview.risk.high,   color:C.error },
                  { label:`Medium (${overview.risk.medium})`, value:overview.risk.medium, color:C.warning },
                  { label:`Low (${overview.risk.low})`,    value:overview.risk.low,    color:C.success },
                ]}/>
                <div style={{ marginTop:20, padding:"12px 16px", background:C.bg, borderRadius:8 }}>
                  <div style={{ fontSize:12, color:C.muted, marginBottom:6 }}>Portfolio risk score</div>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ flex:1, height:10, background:C.border, borderRadius:5, overflow:"hidden" }}>
                      <div style={{ height:"100%", width:`${overview.risk.avg_score}%`,
                        background:overview.risk.avg_score>=67?C.error:overview.risk.avg_score>=34?C.warning:C.success, borderRadius:5 }}/>
                    </div>
                    ...
                    <div key={item.clause_type}>
                      ...
                      <div style={{ height:6, background:C.border, borderRadius:3, overflow:"hidden" }}>
                        <div style={{ height:"100%", width:`${item.pct}%`,
                          background:item.avg_risk>=67?C.error:item.avg_risk>=34?C.warning:C.primary, borderRadius:3 }}/>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:24, marginBottom:20 }}>
              <div style={{ marginBottom:20 }}>
                ...
                <p style={{ fontSize:13, color:C.muted }}>
                  {isFiltered ? "Clause types for this contract" : "Clause types across all contracts"} — count and average risk score
                </p>
              </div>
              <RiskHeatmap matrix={heatmap.matrix} clauseTypes={heatmap.clause_types} />
              {heatmap.ranked?.length > 0 && (
                <div style={{ marginTop:20, paddingTop:16, borderTop:`1px solid ${C.border}` }}>
                  <div style={{ fontSize:13, fontWeight:600, color:C.heading, marginBottom:10 }}>Riskiest clause types</div>
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {heatmap.ranked.slice(0,6).map((item:any,i:number)=>(
                      <div key={item.clause_type} style={{ display:"flex", alignItems:"center", gap:6, padding:"5px 12px", borderRadius:20,
                        ...
                        <span style={{ fontSize:10, fontWeight:700, color:item.avg_risk>=67?C.error:item.avg_risk>=34?C.warning:C.primary }}>#{i+1}</span>
                        <span style={{ fontSize:12, color:C.body, textTransform:"capitalize" }}>{item.clause_type.replace(/_/g," ")}</span>
          ...
          {/* Expiry + Counterparty (portfolio only) */}
          {!isFiltered && (
