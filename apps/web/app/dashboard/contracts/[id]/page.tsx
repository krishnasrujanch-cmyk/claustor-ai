...
export default function ContractDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    const token = getToken();
    fetch(API + "/api/v1/contracts/" + id + "/versions",
      {headers:{Authorization:"Bearer " + (token || "")}})
    .then(r=>r.json())
    .then(d=>{
      const v = (d.versions||[]).sort((a,b)=>(b.version_number||1)-(a.version_number||1));
      setVersions(v);
    }).catch(()=>{});
  }, [id]);
        ...
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
            ...
            {versions.length > 1 ? (
              <select value={id}
                ...
                style={{fontSize:12,fontWeight:700,padding:"3px 10px",borderRadius:20,
                  border:"1px solid #5B4BFF",color:"#5B4BFF",background:"#EEF0FF",cursor:"pointer"}}>
                {versions.map((v:any)=>(
                  <option key={v.id} value={v.id}>
                    {v.is_latest?"✅ LATEST · ":""}v{v.version_number||1}
              ...
              <span style={{fontSize:12,fontWeight:700,padding:"3px 10px",
                borderRadius:20,background:"#EEF0FF",color:"#5B4BFF"}}>
            ...
            {versions.length > 1 ? (
              <select value={id}
                onChange={e=>router.push("/dashboard/contracts/" + e.target.value)}
                style={{fontSize:12,fontWeight:700,padding:"3px 8px",borderRadius:20,
              ...
              <span style={{fontSize:12,fontWeight:700,padding:"3px 8px",borderRadius:20,
                background:"#EEF0FF",color:"#5B4BFF"}}>v1</span>
            )}
          </div>
        </div>
