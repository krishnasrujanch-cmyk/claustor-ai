        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:12, marginBottom:20 }}>
          {industries.map(ind => (
            <div key={ind.id}
              ...
              style={{
                padding:"14px 16px",
                border:`2px solid ${selectedIndustry===ind.id?C.primary:C.border}`,
                borderRadius:10,
                background: selectedIndustry===ind.id ? C.primaryLight : C.surface,
                cursor:"pointer",
                transition:"all 0.15s",
              }}>
              <div style={{ fontSize:20, marginBottom:6 }}>{ind.icon}</div>
              <div style={{ fontSize:14, fontWeight:700, color:C.heading, marginBottom:2 }}>{ind.label}</div>
