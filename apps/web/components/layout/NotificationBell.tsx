...
export function NotificationBell() {
  ...
  const totalAlerts = (alerts?.renewals?.length || 0) + (alerts?.obligations?.length || 0);
  const totalCount  = notifications.length + totalAlerts;
  const urgentCount = (alerts?.summary?.urgent || 0) +
    notifications.filter(n => n.severity === "high" || n.severity === "error").length;
  ...
  return (
    <div ref={ref} style={{position:"relative",flexShrink:0}}>
      <button onClick={()=>{ setOpen(o=>!o); if(!open) load(); }}
        ...
        {totalCount > 0 && (
          <span style={{position:"absolute",top:-4,right:-4,minWidth:18,height:18,
            borderRadius:9,background:urgentCount>0?"#EF4444":"#F59E0B",
      ...
      {open && (
        <div style={{position:"fixed",top:60,left:200,width:360,
          background:"#FFFFFF",border:"1px solid #E5E7EB",borderRadius:12,
          boxShadow:"0 8px 32px rgba(0,0,0,0.16)",zIndex:9999,overflow:"hidden"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              {urgentCount > 0 && (
                <span style={{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,
                  background:"#FEF2F2",color:"#DC2626"}}>{urgentCount} urgent</span>
              )}
          ...
          {loading ? (
            ...
            <div style={{padding:32,textAlign:"center"}}>
              ...
              <p style={{fontSize:13,color:"#6B7280",margin:0}}>All caught up!</p>
            </div>
          ) : (
            <div style={{maxHeight:400,overflowY:"auto"}}>
              ...
              {notifications.map(n=>(
                <Link key={n.id} href={`/dashboard/contracts/${n.contract_id}`}
                  onClick={()=>setOpen(false)}
                  style={{display:"block",padding:"12px 16px",
                    borderBottom:"1px solid #F3F4F6",textDecoration:"none",
                    background:n.severity==="error"?"#FEF2F2":n.severity==="high"?"#FEF2F2":
                              n.severity==="medium"?"#FFFBEB":"#FFFFFF"}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:4}}>{n.title}</div>
              ...
              {alerts?.renewals?.map((r:any)=>(
                <Link key={r.id} href={`/dashboard/contracts/${r.id}`}
                  onClick={()=>setOpen(false)}
                  style={{display:"block",padding:"12px 16px",borderBottom:"1px solid #F3F4F6",
                    textDecoration:"none",background:r.urgency==="urgent"?"#FEF2F2":"#FFFFFF"}}>
                  <div style={{fontSize:11,fontWeight:700,color:r.urgency==="urgent"?"#DC2626":"#D97706",marginBottom:4}}>
                    {r.urgency==="urgent"?"🚨":"⏰"} Renewal in {r.days_until_expiry} days
                  </div>
              ...
              {alerts?.obligations?.slice(0,4).map((o:any)=>(
                <Link key={o.id} href="/dashboard/obligations"
                  onClick={()=>setOpen(false)}
                  style={{display:"block",padding:"12px 16px",borderBottom:"1px solid #F3F4F6",
                    textDecoration:"none",background:o.urgency==="urgent"?"#FFFBEB":"#FFFFFF"}}>
                  <div style={{fontSize:11,fontWeight:700,color:o.urgency==="urgent"?"#DC2626":"#D97706",marginBottom:4}}>
                    📅 Due in {o.days_until_due} days
